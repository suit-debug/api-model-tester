...
A Windows PowerShell GUI tool for fetching and testing OpenAI / Anthropic / OpenAI-compatible API models.

一个 Windows 上的 PowerShell + WinForms 小工具：填一个 Base URL 和 API Key，拉到模型列表后并发把每个模型打一遍，看谁真的能用、谁只是 HTTP 200 但内容为空。

**不编译任何代码、不安装任何依赖、除了你填的那个地址之外不向任何地方发包。**

![API Model Tester](docs/images/api-model-tester.png)
...
- **HTTP status / latency / TTFB / finish reason** —— 每个模型逐项记录，另含 reasoning tokens 与内容预览
- **Compatibility Warning** —— HTTP 200 但内容异常（空内容 / 只有 reasoning / 结构不兼容）不会被误判为 OK
- **CSV / text report export** —— 导出明细 CSV、复制测试报告、复制可用模型清单
- **API keys kept in memory** —— API Key 只在进程内存中，不由本程序主动持久化保存

> 不同供应商与中转站的响应结构差异很大。本工具只按下面写明的规则去解析、并如实报告看到的结果，**不保证对所有 OpenAI-compatible 供应商或所有代理/中转站都兼容**。
...
打分规则：200 且有内容 = 100 分（立即停止探测）；200 但内容异常 = 95；429 = 45；400/422 = 40（报错里点名了模型则 50）；401/403 = 35（路由存在，鉴权/协议头问题）；5xx = 10；404/405 = 0。取最高分，同分按探测顺序。
...
**HTTP 200 不一定代表完全兼容。** 以下情况会标成 `Compatibility Warning`，而不是 OK：
...
1. API Key 只存在于进程内存里的输入框，**不由本程序写入**日志、CSV、报告、配置、错误日志。
...
4. 所有要显示 / 导出的文本统一走 `Mask-Secret`：先精确替换真实 Key，再兜底处理 `sk-xxxx`、`Bearer xxx`、`api_key: xxx` 形态，统一显示成 `sk-abc...xyz`（UI 里的 Key 输入框默认是密码遮挡，截图里的点号就是这个效果）。
...
这是 **151 项断言**，覆盖：语法解析、URL 规范化（含「绝不出现 /v1/v1」）、密钥遮挡、HTTP 状态映射、四类协议响应解析（含畸形 JSON / 空内容 / 只有 reasoning / SSE）、真实往返（打本地假服务器，覆盖 200/400/401/403/404/408/429/500/502/503/524/本地超时）、兼容性重试、Auto Detect 有界探测、并发上限（假服务器侧观测 + 引擎侧峰值 + 耗时对比）、Stop 立即生效、Key 不落盘、报告与 CSV 结构。
...
- `HTTP 200` + `Compatibility Warning` 且备注「只有 reasoning / thinking 内容」：部分中转站的 chat 端点对最小 prompt 只吐思考内容、不吐最终答案，属于供应商差异，不代表端点不可用 —— 换个协议（例如 Responses）或换个模型再看。
